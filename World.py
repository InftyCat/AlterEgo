...
class World :
    ...
    def createker(self,x1,y1,x2,y2):
        ...
        istart = list(filter (lambda  i : Bsc.comPnts(ax.segments[i].src() , xstart) ,  range(len(ax.segments))))
        iend = list(filter (lambda i : Bsc.comPnts(ax.segments[i].trg() , xend) , range(len(ax.segments))))
        if (len(istart) * len(iend) == 0) :
